...
export const renderSkillList = (skillList, heading) => (
  <>
    <h3
      className={`mb-2 text-center font-sans text-2xl font-bold uppercase tracking-widest md:text-left`}
    >
      {heading}
    </h3>
    <div className="mx-auto mb-10 flex max-w-6xl flex-wrap justify-center">
      {skillList.map((skill) => (
        <div
          key={skill.skill}
          className="mb:m-5 m-1 flex flex-col items-center justify-center rounded-3xl bg-slate-50/50 p-2 hover:shadow-2xl md:m-3 md:p-5"
        >
          <Image
            src={skill.img}
            width={100}
